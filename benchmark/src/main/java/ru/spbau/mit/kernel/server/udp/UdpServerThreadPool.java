...
public class UdpServerThreadPool extends Server {
    private static final Logger logger = Logger.getLogger(UdpServerThreadPool.class.getName());
    private static final int N_THREADS_ON_TREAD_POOL = 4;
    private final ExecutorService threadPool = Executors.newFixedThreadPool(N_THREADS_ON_TREAD_POOL);
    private Thread acceptThread = null;
    private volatile DatagramSocket socket = null;

    private void processClient(DatagramPacket packet, long requestProcessingTimeStart) {
        try {
    ...
    private void accept() {
        try {
            while (!Thread.interrupted()) {
                ...
                threadPool.execute(() -> processClient(packet, System.currentTimeMillis()));
            }
        } catch (IOException e) {
